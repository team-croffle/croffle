...
- `release` ("Croffle Release"): manual only. Inputs: `release_type` (patch|minor|major|rc), `version` (exact, no `v`), `version_suffix` (e.g. `rc.1`), `draft` (default false), `dry_run`. Bumps `apps/desktop/package.json`, commits, tags `v<version>`, creates one draft Release first, then mac/linux/win electron-builder jobs attach unsigned installers. Release titles: `Pre-Release vX.Y.Z-rc.A` for rc, `Release vX.Y.Z` for stable. Note: electron-updater cannot see draft releases.
