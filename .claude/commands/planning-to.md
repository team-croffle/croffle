...
2. Walk roadmap versions in semver order from the first unplanned one up to the target. For each: read roadmap + code, write `.ai/plan/<version>_<feature>.md` (`상태: ready`) and `.ai/work/<version>_<NN>-<task>.md`. Skip versions that already have a plan and say so.
3. Keep numbering per version; later versions may depend on earlier ones (note it in `depends` as `"<version>/NN"`).
