...
1. Find the current work version and its first work file with `status: todo` whose `depends` have all finished (their work files no longer exist), lowest NN first. If none: report the version state (all done, or which items are blocked) and stop.
