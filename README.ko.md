...
- **일정**: 우선순위, 반복, 색상 라벨, 태그, 일정별 알림 여러 개, JSON 가져오기/내보내기
- **조회 후 편집**: 일정을 클릭하면 반복 요약이 포함된 조회 화면이 열리고, 버튼 하나로 편집으로 전환
- **데스크톱 최적화**: 넓은 화면과 단축키에 맞춘 UX

### 개인화

- **테마**: 라이트 / 다크 / 시스템. 타이틀 바 테마 버튼을 우클릭해 바로 선택
- **강조 색상**: 설정에서 브랜드 UI(및 톤) 변경
- **시작**: OS 로그인 시 실행, 트레이로 최소화 시작 지원
- **창**: 닫기 버튼을 종료 / 트레이로 숨기기 중에서 선택(처음 한 번 질문), `Alt+B` / `Alt+Shift+B`로 좌·우 사이드바 토글
