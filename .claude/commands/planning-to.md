...
2. Walk roadmap versions in semver order from the first unplanned one up to the target. For each: read roadmap + code, write `.ai/plan/<version>.md` (`상태: ready`) and `.ai/work/<version>/NN-<slug>.md`. Skip versions that already have a plan and say so.
